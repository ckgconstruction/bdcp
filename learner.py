...
class Learner(nn.Module):
    ...
    def load_gradients(self, names, grads):
        ...
        for n, g in zip(names, grads):
            if model_params[n].grad is None:
                continue
            model_params[n].grad.data.add_(g.data)
    ...
    def forward_meta(self, batch_query, batch_support, progress, inner_steps):
        ...
        for task_id in range(task_num):
            self.inner_update(batch_support[task_id], lr_inner,
    ...
    def batch_test(self, data, span_all=None, span_all_norm=None):
        ...
        for i in range((N - 1) // B + 1):
            ...
            tmp_l, tmp_el, tmp_loss, tmp_eval_type_loss = self.model.forward_wuqh(span_all=span_all,
                                                                                  span_all_norm=span_all_norm,
                                                                                  **tmp)
    ...
    def evaluate_meta_(
        ...
        for item_id in tqdm(range(corpus.n_total)):
        ...
        save_pkl(f'./visual/source/span_all_{set_type}_1.pkl', span_all)
        save_pkl(f'./visual/source/span_all_norm_{set_type}_1.pkl', span_all_norm)
        save_pkl(f'./visual/source/span_all_{set_type}_2.pkl', span_all2)
        save_pkl(f'./visual/source/span_all_norm_{set_type}_2.pkl', span_all_norm2)
