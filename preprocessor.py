...
class Corpus(object):
    def __init__(
        ...
        print("Construct the transition matrix via [{}] scheme...".format(viterbi))
        ...
        if viterbi == "none":
            ...
            if self.n_labels == 3:
                self.transition_matrix[2][0] = -10000
        ...
        self.tasks = self.read_tasks_from_file(data_fn, update_transition_matrix, concat_types, dataset)
    ...
    def read_tasks_from_file(
        ...
        print("Reading tasks from {}...".format(data_fn))
        print("  update_transition_matrix = {}".format(update_transition_matrix))
        print("  concat_types = {}".format(concat_types))
        ...
        task_tmp = []
        for task_id, json_str in enumerate(tqdm(json_list)):
            if task_id % 1000 == 0:
                self.logger.info("Reading tasks %d of %d", task_id, len(json_list))
            if task_id % 1000 == 0:
                print("Reading tasks %d of %d", task_id, len(json_list))
            task = json.loads(json_str) if dataset != "Domain" else json_str
            task_tmp.append(task)
    ...
    def _convert_example_to_feature_(
        ...
        return (
            InputFeatures(
                ...
                e_type_mask=np.array(e_type_mask),
